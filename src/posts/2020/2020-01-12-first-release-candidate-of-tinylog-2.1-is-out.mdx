...
Already since the first version, tinylog loads the configuration from the properties file `tinylog.properties` automatically. With the new version, tinylog supports also `tinylog-test.properties` as configuration for tests and `tinylog-dev.properties` as configuration for development. This makes it easier to use different configurations for different environments. Details can be found in the [configuration documentation](/configuration/).
