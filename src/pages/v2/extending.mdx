---
title: Extending
description: 'Learn to extend tinylog 2: Develop custom writers, policies, logging APIs, and other custom extensions. Enhance tinylog with your own powerful features.'
...
The first method `getContextProvider()` returns a provider for the thread-based [mapped diagnostic context values](/v2/logging/#context-values). In the above example, a `NopContextProvider` is registered, which is part of tinylog's API artifact and simply ignores all registered context values.
